...
class UninstallTest
{
    ...
    private function testGetExtensionId()
    {
        $query = $this->db->getQuery(true)
            ...
            ->where($this->db->quoteName('element') . ' = ' . $this->db->quote('com_j2store_cleanup'))
        ...
        if ($extensionId) {
            ...
            $this->fail("Component not found in database");
        }
    }
    ...
    private function testUninstallComponent()
    {
        ...
        try {
            ...
            if ($installer->uninstall('component', $extensionId)) {
                $this->pass("Component uninstalled successfully");
            } else {
                $this->fail("Component uninstallation failed");
    ...
    private function testVerifyUninstall()
    {
        ...
        if ($count == 0) {
            $this->pass("Component removed from database");
        } else {
            $this->fail("Component still exists in database");
        }
        
        // Check if admin files are removed
        $adminFile = '/var/www/html/administrator/components/com_j2store_cleanup/j2store_cleanup.php';
