#!/bin/bash
#
# Install SwissQRCode component (runs inside container)
#
...
DB_PREFIX=$(grep "dbprefix" /var/www/html/configuration.php | cut -d"'" -f2)
echo "Database prefix: $DB_PREFIX"

# Ensure namespace column exists
echo ""
echo "Ensuring namespace column exists..."
mysql -h joomladb -ujoomla -pjoomla_secure_pass_2024 --skip-ssl joomla_db <<EOF 2>/dev/null || true
ALTER TABLE ${DB_PREFIX}extensions ADD COLUMN namespace VARCHAR(255) NOT NULL DEFAULT '' AFTER element;
EOF
echo "✅ Namespace column ready"

# Register extension
echo ""
echo "Registering extension in database..."
mysql -h joomladb -ujoomla -pjoomla_secure_pass_2024 --skip-ssl joomla_db <<EOF
