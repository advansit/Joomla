...
echo "Joomla initialized. Installing dependencies..."
sleep 5
...
# 4. Enable all newly installed plugins
echo ""
...
echo "Using parent menu id: $ROOT_ID"
...
-- Shop overview menu item (published=1, com_j2store) — OSMap hooks here
...
-- published=-2 menu items for products (children of shop menu item 9001)
INSERT INTO ${DB_PREFIX}menu
    (id, menutype, title, alias, link, type, published, parent_id, level, component_id, browserNav, access, img, template_style_id, params, lft, rgt, home, language, client_id)
VALUES
...
-- J2Store product records linked to the articles
...
if [ $? -eq 0 ]; then
    echo "✅ Test fixtures inserted (2 products, shop menu item, 2 product menu items)"
else
