...
class UninstallTest
{
    ...
    public function run(): bool
    {
        ...
        $result = $this->db->query("SELECT extension_id FROM {$this->dbPrefix}extensions WHERE element = 'productcompare' AND type = 'plugin' AND folder = 'j2commerce'");
        ...
        $this->test('Plugin removed from #__extensions', function () {
            $result = $this->db->query("SELECT COUNT(*) as cnt FROM {$this->dbPrefix}extensions WHERE element = 'productcompare' AND folder = 'j2commerce'");
            $row = $result ? $result->fetch_assoc() : null;
            return $row && (int) $row['cnt'] === 0;
        });

        $this->test('Plugin files removed', function () {
            return !file_exists('/var/www/html/plugins/j2commerce/productcompare/src/Extension/ProductCompare.php');
        });
